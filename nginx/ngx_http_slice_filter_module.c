...
ngx_http_slice_header_filter(ngx_http_request_t *r)
{
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice response range: %O-%O/%O",
                   cr.start, cr.end, cr.complete_length);

    slcf = ngx_http_get_module_loc_conf(r, ngx_http_slice_filter_module);

    //garfield reset end
    if(cr.start + (off_t) slcf->size > cr.end) {
       end = cr.end;
    }else{
    ...
    if (cr.start != ctx->start || cr.end != end) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0,
                      "unexpected range in slice response: %O-%O  end:%O",
                      cr.start, cr.end,end);
        return NGX_ERROR;
    }
...
//garfield 
    ctx->complete_length = cr.complete_length;
...
ngx_http_slice_body_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (ngx_http_subrequest(r, &r->uri, &r->args, &ctx->sr, NULL,0)!= NGX_OK)
    {
        return NGX_ERROR;
    }
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest 几个变量start:%O end:%O,complete_length:%O", ctx->start,ctx->end,ctx->complete_length);


   //garfield 修复slice 最后一片小于配置的切片大小 发送的range 请求应为真实的range大小
   if(ctx->start + (off_t) slcf->size  >  (ctx->complete_length - 1) ) {
        ctx->range.len = ngx_sprintf(ctx->range.data, "bytes=%O-%O", ctx->start,
                                    ctx->end -1 )
                       - ctx->range.data;
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest 发送实际大小: \"%V\"", &ctx->range);

   } else {

 if(ctx->jump_range == 1 && ctx->start == 2 ) {
	 //第二次的subrequest range 范围 处理俄日第一片与0-1 range 的差值
       ctx->range.len = ngx_sprintf(ctx->range.data, "bytes=%O-%O", ctx->start,
                                  ctx->start + (off_t) slcf->size - 1 - 2)
                             - ctx->range.data;
       ctx->jump_range = 3;
 } else {
	 //多片的情况 非0-1 片  非第二片  非最后一片 的中间片
      ctx->range.len = ngx_sprintf(ctx->range.data, "bytes=%O-%O", ctx->start,
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest 发送增加一片的大小: \"%V\"", &ctx->range);
   ...
   */
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest: \"%V\"", &ctx->range);
    ...
    ngx_http_variable_value_t *v, uintptr_t data)
{
	...
	if(ctx->jump_range == 0) {
		//切片的第一次subrequest 请求
           ctx->jump_range = 1;
           ctx->range.data = p;
           ctx->range.len = ngx_sprintf(p, "bytes=0-1")
                         - p;
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest range value 首次获取总长度: \"%V\"", &ctx->range);
	}else{
		//走不到这个逻辑
	   ctx->jump_range = 2;
           ctx->start = slcf->size * (ngx_http_slice_get_start(r) / slcf->size);
	   ctx->jump_range = ctx->start;
           ctx->range.data = p;
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest range value 非首次: \"%V\"", &ctx->range);
	}
    } else {
      ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest range value ctx==NULL: \"%V\"", &ctx->range);
    }

    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   "garfield http slice subrequest range value: \"%V\"", &ctx->range);
